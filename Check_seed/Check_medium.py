...
def medium_check_All(route, medium, info, title,soup):
    ...
    def info_check():
        if medium == 'encode' or medium == 'Encode':
            medium_type_1 = re.search('(?=Blu-ray|BluRay).*x264', title,re.I)
            medium_type_2 = re.search('(?=Blu-ray|BluRay).*x265', title,re.I)
            medium_type_3 = re.search('Encoding settings', info, re.I)
            medium_type_5 = re.search('编码库', info, re.I)
            if medium_type_1 or medium_type_2 or medium_type_3  or medium_type_5:
                return True
        ...
        if medium == 'Remux':
            ...
            medium_type4_4=re.findall('(?=Bit.*rate|码率).*:(.*) (?=Mbps|Mb/s)',info,re.I)
            try:
            ...
            if medium_type4_3:
                if int(medium_type4_3[0])>20480:
                    return True
        if medium=='DVD':
            medium_type7=re.search(medium,title,re.I)
            if medium_type7:
                return True
    def Number_of_files():
        if medium == 'Blu-ray' or medium == 'UHD Blu-ray':
